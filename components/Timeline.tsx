...
export default function Timeline({ items }: TimelineProps) {
  ...
  return (
    <div className="timeline font-mono">
      ...
      {items.map((item, i) => (
        ...
        >
          ...
          <div className="timeline-title group-hover:text-white transition-colors flex items-center gap-2">
            {item.title}
            <span className="h-[1px] flex-1 bg-cyan-900/30 group-hover:bg-cyan-500/30 transition-all"></span>
          </div>
          ...
          <div className="absolute -left-[35px] top-1 text-[8px] text-cyan-700 opacity-0 group-hover:opacity-100 transition-opacity">
      ...
      <div className="h-4 border-l-2 border-dashed border-cyan-900/30 ml-[20px] mt-2"></div>
